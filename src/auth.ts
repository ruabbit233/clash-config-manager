...
export const verifyToken = async (secret: string, token: string): Promise<TokenPayload | null> => {
  ...
  try {
    ...
    if (typeof decodedPayload.exp !== 'number' || typeof decodedPayload.iat !== 'number') {
      return null
    }
