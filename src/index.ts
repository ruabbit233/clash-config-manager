...
const createYamlResponse = async (c: Context<{ Bindings: Env }>, asDownload: boolean) => {
...
const resolveToken = async (c: Context<{ Bindings: Env }>): Promise<string | null> => {
...
const setAuthCookies = (c: Context<{ Bindings: Env }>, token: string, expiresAt: string) => {
...
app.patch('/api/versions/:id', async (c) => {
  const id = c.req.param('id')
